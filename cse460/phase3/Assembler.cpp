...
//VirtualMachine.cpp
//Update:for phase2 nothing much changed except outputting which prog is compiled.
/////////////////////////////////////////////////////////////////////////
...
void Assembler::compile(string filename)
{
    ...
    ofile.replace(ofile.end()-2, ofile.end(), ".o");//create name for object file
    ofstream outfile;
    outfile.open(ofile.c_str(), ios::binary);
	...
	*/
           ...
           else
           {
	     	...
	     	if(iType == dm && i == 0)
	           aError("Address cannot be negative");
                    ...
                    aError("Address cannot be negative");
		if(opd > 128 || opd < -128)
		    aError("Address/constant is too big for 8 bit format");
	...
	mcByte &= 15;
	mcByte += 48;
	charByte = (char)mcByte;
	outfile.put(charByte);
	}
        }
	}	
    ...
    infile.close();
    outfile.close();
      cout << "Assembly: " << filename << "completed!" << endl;
    return;
}
